...
- Slash commands require the host command service. `subagent_role` role delegation requires agents, tools, LLM, subagent and session-persistence services; it starts a durable background child by default, applies the saved persona and any route the role declares or the call overrides, and returns the child id without waiting. `run_in_background: false` runs the child once in the foreground and returns its report; `run_in_background: true` runs it as a tracked job instead and additionally needs the host jobs registry (`@deepseek-ai/dsh-jobs` and `@deepseek-ai/dsh-tool-jobs`).
