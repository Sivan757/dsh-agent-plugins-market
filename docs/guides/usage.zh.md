...
- 斜杠命令需要宿主命令服务。`market_agent` 角色委派需要 tools、LLM 和 subagents 服务；它将保存的角色指令、模型/provider 和工具限制应用到真实子代理，并等待结果。
...
市场页顶部的源胶囊是等宽网格：默认折到两行，用「展开全部」/「收起」就地展开；当前选中的源始终可见，编辑与删除操作保持可用。
...
自建条目支持 frontmatter `disabled: true`，停止注册但保留文件。命令将正文转交给模型，并把 `$ARGUMENTS` 替换为调用时的文本。自建角色也会注册为 `persona-<name>` 技能。

项目维度的状态和 checkout 位于 `<project>/.dsh/agent-plugins/`。原生 `.claude/`、`.agents/` 技能与代理直接读取，无需安装状态。同名时，项目技能优先于已安装的用户套件技能，自建面板技能优先级低于套件技能。条目被遮蔽时可通过改名解决。
...
一个来源可以包含多种布局方言。清单优先级决定套件身份，marketplace 条目决定主要套件集合，并按支持的规则补充发现。远程引用卡片不能直接安装，需要先添加对应仓库为来源。
