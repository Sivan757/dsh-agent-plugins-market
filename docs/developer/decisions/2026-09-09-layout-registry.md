...
1. The registry and table-driven tests cover `.claude`, `.agents`, `.codex`, `.cursor`, `.kimi`, `.zcode`, `.qoder` and `.github`. Portable and Universal content shares `.agents`; hooks are the one non-Markdown surface that layout also carries, through its own `hooks.json` candidates. The adapter does not invent a native directory for every manifest dialect. These are supported DSH surfaces, not a reimplementation of each original client's rule/trust/model engine.
