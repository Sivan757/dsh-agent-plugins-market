...
The host namespace `dsh-agent-plugins-market` exposes `scanProjectLayouts: boolean`, default `false`: native project layouts join discovery only after the user turns the switch on, so a project cannot inject its resources into a session silently ([project layouts are opt-in](../../../.agents/notes/implemented/architecture/2026-09-13-project-layouts-opt-in.md)). The setting controls native project directory discovery only. Configured sources and `.dsh/agent-plugins` install state retain their meaning. Changes invalidate discovery and project snapshots, then notify providers and reconciliation. The value participates in the discovery fingerprint so an older scan cannot supply a differently configured snapshot.
...
- `project-runtime.ts` shares agent attachment, serialized refresh and disposal across independent command and MCP children. MCP reads project `.mcp.json`, the `.agents` layout's `mcp.json`, Cursor `.cursor/mcp.json`, and Qoder's project/local settings MCP tables. It ignores unrelated settings, resolves relative commands from the project root, and rejects malformed overriding layers. Each layout remains a separate DSH suite; shared `.mcp.json` is read once, not once per client. Cross-layout names are namespaced rather than reproducing one client's cross-source priority.
- MCP tools mount in the agent's injected tools scope. A server's derived name is one `suite__server` identity in every dimension: the host keeps a scope's own registrations in that scope's layer and lets them shadow inherited ones, so two agents — or a project and the user dimension — mount the same server key independently without a per-session suffix. One identity also means one stored OAuth grant per server, so a project mount reads back the grant the user's own mount wrote; see the [session-identity note](../../../.agents/notes/implemented/bug-fix/2026-09-13-mcp-identity-without-session-namespace.md). The backend setting applies to project mounts as well.
- A cwd with no `.git` ancestor resolves to itself as its project root, so a session started in the harness home would land its project dimension on the user dimension root. That directory is read as an empty project catalog instead: the user's installs belong to the user dimension, and presenting them again per session mounted every user-level server a second time.

## Runtime coverage

Native Claude/Qoder settings hooks, the `.agents` layout's standalone `hooks/hooks.json` or `hooks.json` (bare event table or a `hooks` key), and enabled ZCode configuration hooks now use the existing execution bridge under each agent. Normalization rejects malformed groups, reports unsupported events/types, deduplicates identical hooks and preserves disable flags. The bridge receives a private runtime-generated JSON snapshot containing hooks only; project files are never rewritten. Fingerprint changes remount hooks, and teardown removes the snapshot. This is a derived runtime artifact, not a copied plugin installation.

ZCode MCP reads `mcp.servers` from `zcode.json` followed by `.zcode/config.json`. `.agents/mcp.json` belongs to the `.agents` layout itself, which reads it as that layout's own `mcpServers` document; the earlier ZCode empty-table fallback is gone, so each layout reads only its own files ([`.agents` surface parity](../../../.agents/notes/implemented/architecture/2026-09-22-agents-layout-full-surface-parity.md)). Disabled entries do not run. The compatibility report has been regenerated from a freshly compiled scanner and uses the shared marketplace registry.

1. The registry and table-driven tests cover `.claude`, `.agents`, `.codex`, `.cursor`, `.kimi`, `.zcode`, `.qoder` and `.github`. Portable and Universal content shares `.agents`; besides Markdown resources that layout also carries its own `hooks.json` candidates and its own `mcp.json`. The adapter does not invent a native directory for every manifest dialect. These are supported DSH surfaces, not a reimplementation of each original client's rule/trust/model engine.
...
An isolated DSH profile using the installed base/web bundles and this linked plugin verifies persistence through the real settings UI while the default was `true`: `false` after clicking the switch, still `false` after a browser reload, and still `false` in a new browser after terminating and restarting the host process. The default is now `false`, and the explicit `true` value persists the same way. The value is stored under the isolated `DSH_HOME/settings.yaml`; the existing user instance is untouched. Both validation host processes were stopped after the check.
