...
export function RoleMetadataFields(props: { text: string; onChange: (text: string) => void; t: Translate; disabled?: boolean }): ReactNode {
  ...
  try {
    ...
    return h('p', { className: formCss.footerError, role: 'alert' }, props.t('personaMetadataInvalid'), ' ', String(error instanceof Error ? error.message : error))
  }
...
function RoleFields(props: { text: string; onChange: (text: string) => void; t: Translate; disabled?: boolean; fields: ReturnType<typeof readRoleFields> }): ReactNode {
  ...
  return h(
    ...
    h(
      ...
      h('span', null, t('personaProvider')),
      h(
        'select',
        { 'aria-label': t('personaProvider'), value: provider, onChange: (event: { target: HTMLSelectElement }) => selectProvider(event.target.value) },
    ...
    h(
      'label',
      { className: formCss.field },
      h('span', null, t('personaModel')),
      h(
        'select',
        {
          'aria-label': t('personaModel'),
          value: model,
          disabled: provider === '' || loading,
          onChange: (event: { target: HTMLSelectElement }) => props.onChange(updateRoleReasoning(updateFrontmatter(updateFrontmatter(props.text, 'provider', provider), 'model', event.target.value), ''))
        },
    ...
    h(
      'label',
      { className: formCss.field },
      h('span', null, t('personaReasoningEffort')),
      h(
        'select',
        {
    ...
    providerError || modelError || effortError === route
      ? h('div', { role: 'alert', className: `${formCss.footerError} ${formCss.full}` }, t('personaCatalogError'), h('button', { type: 'button', onClick: () => setRevision(value => value + 1) }, t('refresh')))
      : null,
    ignoredRoute ? h('p', { className: `${formCss.hint} ${formCss.full}`, role: 'status' }, t('personaRouteIgnored')) : null,
    // The route the child will actually run with, resolved from the two
    // selectors above.
    h(
      'p',
      { className: `${formCss.hint} ${formCss.full}` },
      props.t('personaRouteCurrent'),
      h('code', null, hasRoute ? `${provider} / ${model}` : props.t('personaInherit'))
    )
  )
}
