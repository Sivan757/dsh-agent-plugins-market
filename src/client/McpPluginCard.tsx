/**
 * The MCP enhancement card on the host's 插件配置 (plugin configuration) tab:
 * one plain switch — ON (default) mounts suite MCP servers through the
 * market's built-in bridge (OAuth authorization, SSE, browser-leg hold); OFF
 * falls back to the host's `dsh-mcp-client` compatibility client.
 *
...
export function McpPluginCard({ t }: McpPluginCardProps): ReactNode {
  ...
  const enhanced = info !== undefined && info.backend === 'builtin'
  ...
  return h(
    ...
    h(
      ...
      h(
        ...
        h('div', { className: css.pluginCardTitle }, t('mcpCardTitle')),
