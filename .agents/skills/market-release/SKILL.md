...
Releases are workflow-owned: release-please (embedded in `.github/workflows/npm-publish.yml`) opens the Release PR against `main`, and merging it auto-tags, auto-creates the GitHub Release, and auto-publishes npm. The full runbook lives at [docs/developer/release/release-process.md](../../../docs/developer/release/release-process.md); read it before acting.
...
6. **Report and close out.** Confirm tag + Release + `npm view @deepseek-ai/dsh-agent-plugins-market dist-tags`, then offer the post-release items from `docs/developer/release/release-v<version>.md` (Highlights on the auto-generated Release body, announcements).
