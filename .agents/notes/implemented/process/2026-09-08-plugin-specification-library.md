...
- **A single long `docs/reference/` document.** Cheaper to write, but it cannot be compiled, cannot be referenced per dialect, and would have to restate the field tables that a schema states mechanically. The repository already has `docs/reference/dsh-plugin-development-standard.md` for a different audience.
