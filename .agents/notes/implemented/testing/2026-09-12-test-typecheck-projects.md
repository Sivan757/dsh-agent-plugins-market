...
- **Every mutating route coerced untrusted JSON with `String()`.** `{"url": {"href": "x"}}` registered a source whose url was the literal `"[object Object]"`, and `{"id": {}}` passed the `id === ''` check before reaching the registry. The sites now narrow with `typeof` where that is behaviour-preserving; converting the rest to strict validation is a behaviour change and is not in this note.
...
- **A test name that claimed a negative it never asserted.** The prompt for the type-checked rules is what made a maintainer read the file closely enough to notice.
- **`noUncheckedIndexedAccess` is off in all four projects**, which is why roughly a hundred removed non-null assertions were decorative: `arr[0]` is typed `T`, so `arr[0]!` guarded nothing. Enabling it is a separate change — measured at **255 errors in the test project alone**.
...
- `pnpm run typecheck` runs all four projects; `check:quick` and `check:refactor` are the two gates.
