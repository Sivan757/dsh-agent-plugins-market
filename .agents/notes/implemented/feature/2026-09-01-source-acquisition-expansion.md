...
- **Adoption (manual-clone repair).** `addSource` first checks the candidate ids' checkout directories: a directory whose `origin` remote matches the input URL under `canonicalGitUrl` equality is registered as-is with `adopted: true` — no clone, no rename, no invented id. `sources/adopt` registers any unmanaged `.sources/` checkout explicitly (non-git directories become `local` sources). The overview payload reports `unmanaged` checkouts and the client renders an adoption row. Adopted and local sources are user-owned: URL changes never delete their directories. Since the [workspace-tabs note](2026-09-06-workspace-tabs-user-panels.md), `removeSource(id, deleteCheckout)` gains an _opt-in_ physical deletion (a confirmation-dialog checkbox): it removes any `.sources/<id>` checkout, adopted included, because that directory is manager-owned storage; only a `local` source pointing outside `.sources/` is registration-only. The client renders adopted sources as ordinary chips — since the [source-strip fold note](2026-09-08-source-strip-fold.md) they carry no adoption badge.
