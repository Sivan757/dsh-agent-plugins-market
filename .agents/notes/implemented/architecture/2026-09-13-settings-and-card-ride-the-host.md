...
Host seams. `src/runtime/mcp-client/host-seams.ts` re-exports `MAX_TIMER_DELAY_MS`, `scrubbedParentEnv`, `credentialKey`, and the image-admission vocabulary from `@deepseek-ai/dsh-timeout`, `dsh-subprocess`, `dsh-credentials`, and `dsh-attachment`. The tool observation reads `ctx.tools.schemas()`. The host locale preference reads `locale.preference` off the settings service, keeping the `settings.yaml` parse only as the fallback for compositions that do not mount the locale plugin.
