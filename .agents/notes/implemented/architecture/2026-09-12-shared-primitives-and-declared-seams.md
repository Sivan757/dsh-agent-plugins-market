...
Alongside the structure: seven catalog exports with no consumer (including three abandoned manifest readers), four dead `Catalog` members, a shadowed `hasSuiteManifest`, five copies of a filesystem probe, 69 CSS classes and 56 locale keys referenced nowhere, three client API helpers, `PanelShell`/`PanelAction`, and thirteen copies of the same fetch boilerplate in `api.ts`.
...
- `src/` is flat in size (22,140 → 22,121 lines). This is a structure change, not a line-count change: the split adds module scaffolding while removing duplication. What moves is the maximum: the largest source file drops from 1,260 to 551 lines, `apply()` from 349 to 183, `src/client/` shrinks by 618 lines, `src/catalog/` by 124.
