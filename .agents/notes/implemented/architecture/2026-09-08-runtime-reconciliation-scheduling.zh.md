...
慢速 MCP 连接不再延迟发起该变更的操作完成：变更在状态落盘后即应答，协调在其后追平，且变更流水线的每个阶段都有界——见[变更流水线与请求等待的有界化](2026-09-13-bounded-change-pipeline-and-request-waits.zh.md)。其他运行时 surface 独立就绪，连续设置或凭据通知不会积累无界目录协调队列。凭据通知继续保守处理，避免丢失首次凭据引用快照建立前收到的更新。
